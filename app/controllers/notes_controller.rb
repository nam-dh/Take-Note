class NotesController < ApplicationController
  ...
  def result
    ...
    query << "%"
    if f == 0
      @notes = Note.where "user_id = ? AND date BETWEEN '?-?-?' AND '?-?-?' AND (content like ? OR title like ?)", @user.id, yfrom, mfrom, dfrom, yto, mto, dto, query, query
    else
      @notes = Note.where "user_id = ? AND course_id = ? AND date BETWEEN '?-?-?' AND '?-?-?' AND (content like ? OR title like ?)", @user.id, f, yfrom, mfrom, dfrom, yto, mto, dto, query, query
    end
